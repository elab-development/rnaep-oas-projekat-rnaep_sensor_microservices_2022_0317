...
exports.setThreshold = async (req, res) => {
  try {
    ...
    console.log(`✅ ALERT: Prag postavljen za zonu "${zone_id}"`);
    res.status(201).json({
      message: 'Prag uspešno postavljen',
      threshold: result.rows[0]
    });
  } catch (error) {
    console.error('❌ ALERT: Greška u setThreshold:', error);
...
exports.getThreshold = async (req, res) => {
  try {
    ...
    console.error('❌ ALERT: Greška u getThreshold:', error);
    res.status(500).json({ error: error.message });
  }
};
...
exports.checkAlerts = async (req, res) => {
  try {
    ...
    console.log('📥 ALERT: Primljen zahtev za proveru alerta');
    ...
    // ===== NOVI LOGOVI ZA DEBUG =====
    console.log(`🔍 ALERT: Tražim prag za zonu: "${zone_id}" (tip: ${typeof zone_id})`);

    // Dohvati prag za ovu zonu
    const threshold = await pool.query('SELECT * FROM thresholds WHERE zone_id = $1', [zone_id]);

    // ===== NOVI LOGOVI ZA DEBUG =====
    console.log(`🔍 ALERT: Broj pronađenih pragova: ${threshold.rows.length}`);
    if (threshold.rows.length > 0) {
      console.log('   Pronađen prag:', JSON.stringify(threshold.rows[0], null, 2));
    }

    if (threshold.rows.length === 0) {
      console.log('ℹ️ ALERT: Nema postavljenih pragova za ovu zonu - vraćam success (200)');
      return res.json({ 
    ...
    if (moisture < t.critical_moisture) {
      console.log(`🚨 ALERT: KRITIČAN nivo! ${moisture}% < ${t.critical_moisture}%`);
    ...
    // Proveri WARNING prag
      ...
      console.log(`⚠️ ALERT: UPOZORENJE! ${moisture}% < ${t.warning_moisture}%`);
      const alert = await pool.query(
        `INSERT INTO alerts (zone_id, type, moisture_value, threshold_value, message)
      ...
      console.log(`✅ ALERT: Sve je u redu, vlažnost ${moisture}% je iznad praga`);
    }

    console.log(`✅ ALERT: Provera završena, kreirano ${alerts.length} alerta`);
    res.json({
    ...
    // ===== POBOLJŠAN LOG ZA GREŠKU =====
    console.error('❌❌❌ ALERT: GREŠKA U checkAlerts ❌❌❌');
    console.error('   Poruka:', error.message);
    console.error('   Stack:', error.stack);
    console.error('   Detalji:', JSON.stringify(error, null, 2));
    res.status(500).json({ 
      error: error.message,
      stack: error.stack // opciono, za debug
    });
  }
};
...
exports.getAlertHistory = async (req, res) => {
  try {
    ...
    console.log(`📋 ALERT: Dohvaćeno ${result.rows.length} alerta za zonu "${zone_id}"`);
    res.json(result.rows);
  } catch (error) {
    console.error('❌ ALERT: Greška u getAlertHistory:', error);
    res.status(500).json({ error: error.message });
  }
};
...
exports.resolveAlert = async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`🔧 ALERT: Resolvujem alert ${id}`);
    ...
    console.log(`✅ ALERT: Alert ${id} resolvovan`);
    res.json({
      message: 'Alert uspešno resolvovan',
      alert: result.rows[0]
    });
  } catch (error) {
    console.error('❌ ALERT: Greška u resolveAlert:', error);
    res.status(500).json({ error: error.message });
  }
};
...
exports.getZonesWithThresholds = async (req, res) => {
  try {
    ...
    console.log(`📋 ALERT: Dohvaćeno ${result.rows.length} pragova`);
    res.json(result.rows);
  } catch (error) {
    console.error('❌ ALERT: Greška u getZonesWithThresholds:', error);
    res.status(500).json({ error: error.message });
  }
};